...
def search(G, start: int, goal: int, weight: str = 'custom_weight') -> SearchResult:
    ...
    Generally expands fewer nodes than unidirectional A* on large graphs.
    """
    if start == goal:
        return SearchResult(path=[start], nodes_expanded=0)

    # Forward frontier: f = g_fwd + h(node, goal)
    # Backward frontier: f = g_bwd + h(node, start)  (reverse graph)
    ...
    while fwd_heap or bwd_heap:
        # Termination: if the best possible path through the frontiers can't beat best_cost
    ...
    full_path = fwd_path + bwd_path[1:]  # avoid duplicating meeting_node
...
def _reconstruct(parent_map: dict, start: int, end: int) -> list:
    """Walk parent pointers from end back to start, then reverse."""
    path = []
    node = end
